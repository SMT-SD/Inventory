...
class DetailedOperationInheritClass(models.Model):
    ...
    location_select = fields.Many2one('stock.location', string='Destination Location', index=True,
                                      required=False)
    ...
    image_medium = fields.Binary(
        "Product Sale image", attachment=True, related='product_id.image')
    image_medium_two = fields.Binary(
        "Product Purchase image", attachment=True, related='product_id.image_medium_two')
    ...
    def check_reserved(self):
        if self.picking_type_id.code == 'internal' and self.is_pick==True:
            for line in self.move_line_ids:
                if line.product_uom_qty <1:
                    print('hello dears')
                    raise ValidationError('you cant not save when reserved is less than 1!!')

    @api.multi
    @api.onchange('location_scan')
    def compute_locations_from_scan(self):
        if self.code == 'internal' and self.is_pick == False and self.location_scan:
                ...
                if check:
                    ...
                    if self.is_pick == False:
                        if self.location_select:
                            ...
                            for rec in self.move_line_ids:
                                list.append(rec.lot_id.name)
                                if ((rec.lot_id.name == self.search_serial) and rec.qty_done>0 and rec.location_dest_id==self.location_select):
...
class StockMoveLineInheritClass(models.Model):
    ...
    production_date = fields.Datetime(string='Production Date', related='move_id.production_date',store=True)
    ...
    @api.multi
    @api.onchange('lot_name', 'qty_done')
