...
class ProductChangeUoM(models.TransientModel):
    ...
    def do_change(self):
        ...
        if active_id:
            ...
            # inlocuire in account move lines
            ...
            domain = [("product_id", "in", variants.ids), ("uom_id", "=", product.uom_id.id)]
            invoice_lines = self.env["account.invoice.line"].search(domain)
            if invoice_lines:
                query_dic = {
                    "ids": tuple(invoice_lines.ids),
                    "product_uom_id": self.uom_id.id,
                }
                query = "UPDATE account_invoice_line SET uom_id = %(product_uom_id)s WHERE id in %(ids)s"
                self._cr.execute(query, query_dic)
            ...
            if stock_move_lines:
                ...
                self._cr.execute(query, query_dic)
            domain = [("product_id", "in", variants.ids), ("product_uom_id", "=", product.uom_id.id)]
            stock_production_lot = self.env["stock.production.lot"].search(domain)
            if stock_production_lot:
                query_dic = {
                    "ids": tuple(stock_production_lot.ids),
                    "product_uom_id": self.uom_id.id,
                }
                query = "UPDATE stock_production_lot SET product_uom_id = %(product_uom_id)s WHERE id in %(ids)s"
                self._cr.execute(query, query_dic)
