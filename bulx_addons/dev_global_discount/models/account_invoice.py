...
class account_invoice(models.Model):
    ...
    apply_discount = fields.Boolean('Discount Applied on Total')
    apply_discount_line = fields.Boolean('Discount Applied Per Line')
    ...
    discount_amount = fields.Float('Discount Amount',store=True,copy=True,compute='compute_total_disc_amount_line')

    @api.one
    @api.depends('invoice_line_ids','discount_type','sale_discount')
    def compute_total_disc_amount_line(self):
        if self.apply_discount == True:
            if self.discount_type =='fixed':
                self.discount_amount = self.sale_discount
            elif self.discount_type =='percent':
                self.discount_amount = (self.sale_discount *self.amount_untaxed) /100
        elif self.apply_discount_line ==True:
            for line in self.invoice_line_ids:
                self.discount_amount +=  line.discount_amount

    @api.multi
    @api.onchange('apply_discount','apply_discount_line')
    def two_discount_apply(self):
        for rec in self:
            if rec.apply_discount==True and rec.apply_discount_line==True:
                raise ValidationError('You can not apply two discounts at the same time')
    @api.one
    @api.depends('invoice_line_ids.price_subtotal', 'tax_line_ids.amount', 'currency_id', 'company_id', 'date_invoice',
                 'type', 'discount_type', 'sale_discount','discount_amount')
    def _compute_amount(self):
        ...
        elif self.apply_discount_line and self.discount_amount:
                dis_amt = self.discount_amount
    ...
    disc_amount = fields.Float('Total After Discount', compute='_compute_amount')
    ...
    def finalize_invoice_move_lines(self, move_lines):
        ...
        total_amount = 0.0
        for inv in self:
            if inv.apply_discount or inv.apply_discount_line:
            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                if refund:
                    refund.write({
                        ...
                        'apply_discount': inv.apply_discount or inv.apply_discount_line,
...
class AccountInvoiceLineInheritBulx(models.Model):
    _inherit = "account.invoice.line"

    discount_type = fields.Selection([('fixed', 'Fixed'), ('percent', 'Percentage')],default='fixed', string='Discount Type')
    dis_amount = fields.Float('Amount/Percentage%')
    discount_amount=fields.Float('Discount Amount',compute='compute_dis_amount_per_line')
    apply_discount_line = fields.Boolean('Discount Applied Per Line',related='invoice_id.apply_discount_line')


    @api.multi
    @api.depends('discount_type','dis_amount')
    def compute_dis_amount_per_line(self):
        for rec in self:
            if rec.discount_type=='fixed':
                rec.discount_amount = rec.dis_amount
            elif rec.discount_type=='percent':
                rec.discount_amount = (rec.price_subtotal *rec.dis_amount) /100
