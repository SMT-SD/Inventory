...
class PaymentAccountbulxInherit(models.Model):
    ...
    type = fields.Selection([('vendor_payment', 'Vendor Payment'), ('customer_payment', 'Customer Payment')],
                            string='Type', required=False, readonly=True)
    ...
    @api.multi
    def button_print_customer_payment(self):
        return self.env.ref('accounting_updates.money_receipt_report').report_action(self)

    @api.multi
    def button_print_vendor_payment(self):
        return self.env.ref('accounting_updates.money_send_report').report_action(self)
